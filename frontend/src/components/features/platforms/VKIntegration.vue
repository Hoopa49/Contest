...
export default {
  ...
  setup() {
    ...
    const stats = computed(() => [
      {
        ...
        value: store.getVKStats?.contestsFound || 0,
        icon: 'mdi-trophy',
        color: 'success'
      },
      {
        title: 'Запросов к API',
        value: store.getVKStats?.apiRequests || 0,
        icon: 'mdi-api',
        color: 'info'
      },
      {
        title: 'Ошибок',
        value: store.getVKStats?.errors || 0,
        icon: 'mdi-alert',
        color: 'error'
      }
    ])
    ...
    const loadSettings = async () => {
      loading.value = true
      try {
        const response = await store.getVKSettings()
        enabled.value = response.enabled
        settings.value = { ...response.settings }
      } catch (error) {
        console.error('Ошибка при загрузке настроек VK:', error)
      } finally {
        loading.value = false
      }
    }
    ...
    const saveSettings = async () => {
      ...
      saving.value = true
      try {
        await store.updateVKSettings({
          enabled: enabled.value,
          settings: settings.value
        })
      } catch (error) {
        console.error('Ошибка при сохранении настроек VK:', error)
      } finally {
        saving.value = false
      }
    }
    ...
    const toggleIntegration = async () => {
      loading.value = true
      try {
        await store.toggleVKIntegration(enabled.value)
      } catch (error) {
        console.error('Ошибка при переключении интеграции VK:', error)
