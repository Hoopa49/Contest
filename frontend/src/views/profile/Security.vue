 ...
 * Управление паролем, двухфакторной аутентификацией и просмотр истории входов
 */
<template>
  <v-container>
    <v-row>
        ...
        <v-card>
          ...
          <v-card-text>
            <v-alert
              ...
              class="mb-4"
            >
              Рекомендуется включить двухфакторную аутентификацию для повышения безопасности аккаунта
            </v-alert>
...
export default {
  ...
  setup() {
    ...
    const getDeviceIcon = (device) => {
      ...
      return icons[device.toLowerCase()] || icons.default
    }
    ...
    onMounted(async () => {
      ...
      try {
        ...
        const twoFactorStatus = await userStore.getTwoFactorStatus()
        twoFactor.enabled = twoFactorStatus.enabled
        if (twoFactor.enabled) {
          twoFactor.backupCodes = twoFactorStatus.backupCodes
        }

        // Загружаем историю входов
        const history = await userStore.getLoginHistory()
